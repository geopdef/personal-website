document.addEventListener("DOMContentLoaded", () => {
  ...
  const sideNavItems = document.querySelectorAll("[data-side-nav], [data-learn-nav]");
  const sidePanels = document.querySelectorAll("[data-side-panel], [data-learn-panel]");

  if (sideNavItems.length && sidePanels.length) {
    const getNavKey = (el) => el.getAttribute("data-side-nav") || el.getAttribute("data-learn-nav");
    const getPanelKey = (el) => el.getAttribute("data-side-panel") || el.getAttribute("data-learn-panel");

    const activateSidePanel = (key) => {
      sideNavItems.forEach((item) => {
        item.classList.toggle("is-active", getNavKey(item) === key);
      });
      sidePanels.forEach((panel) => {
        const match = getPanelKey(panel) === key;
    ...
    sideNavItems.forEach((item) => {
      item.addEventListener("click", () => {
        activateSidePanel(getNavKey(item));
      });
    });
  }
  ...
  let currentMatchPromise = null;  // 保存当前运行中的匹配任务
  ...
  function startProgressAnimation() {
    ...
    progressInterval = setInterval(() => {
      ...
      progress = Math.min(progress, 99);  // 最多到99%，等待完成
  ...
  if (matcherForm) {
    matcherForm.addEventListener("submit", async (e) => {
      ...
      // Show loading state - 隐藏表单，显示加载动画
      matcherForm.style.display = "none";
      loadingSpinner.style.display = "flex";
      loadingSpinner.style.position = "relative";
      loadingSpinner.style.bottom = "auto";
      loadingSpinner.style.right = "auto";
      loadingSpinner.style.zIndex = "auto";
      startProgressAnimation();

      // 在后台运行匹配任务
      currentMatchPromise = matchJobDescription(jd);
      currentMatchPromise
        .then(({ result }) => {
          completeProgress();
          const cardHTML = renderMatchCard(result);
          matchModalContent.innerHTML = cardHTML;
