document.addEventListener("DOMContentLoaded", () => {
  ...
  const projectDetails = {
      ...
      responsibilities: [
        {
          ...
          title: "背景与目标",
          items: [
            "背景：① ARPU 和 CR 的提升逐渐接近阶段性增长瓶颈，团队开始把流量增长作为核心抓手；② 两个卡点——业务平时主要看详情页、列表页的 UV、CR，无法知道用户从哪里进入、经过哪些页面；七条产线数据口径分散，跨产线取数需人工整合多份报表，效率很低",
            "目标：从用户路径视角定位度假流量的来源和转化瓶颈，再将分析中沉淀的口径和维度建设为统一的流量监控体系"
          ]
        },
        {
          tone: "green",
          title: "三个假设",
          items: [
            "先从用户视角梳理完整链路：用户从携程 App 的 DAU 出发，可能经过各入口，进入度假列表页、详情页、资源页、填写页，最终下单成单",
            "假设一：不同前序入口带来的用户质量不同",
            "假设二：用户并不是只经过单一页面就下单，而是存在不同决策路径",
            "假设三：流量问题可能来自不同环节"
          ]
        },
        {
          ...
          title: "沉淀末次归因路径",
          items: [
            "页面归因和用户路径分层：将约 4,600 个 PageID 抽象为业务能理解的页面类型；利用马尔可夫链并结合页面访问频次，识别出 12 个关键用户阶段；使用 Jaccard Distance 和 K-medoids 对用户路径进行聚类，沉淀 8 条核心转化路径",
            "多维漏斗拆解和机会点识别：按主动搜索、首页浏览、收藏/浏览历史复访等划分 8 条核心转化路径的用户类型；对比详情页 UV、CR、唤起率、唤起 CR 和订单贡献，从而发现机会点并提出建议"
          ]
        },
        {
          tone: "purple",
          title: "沉淀长期监控看板",
          items: [
            "参与梳理七条产线的 DAU、页面归属、导购链路和挂单口径",
            "设计从「携程 DAU→成单」的全链路 P0 到 P3 指标体系",
            "输出日活表、导购链路表等数仓建设需求",
            "利用 AI 实现智能监控指标异动以及异动归因功能"
          ]
        }
      ],
      results: [
        "推动全链路流量看板上线，覆盖 7 条产线、11 个核心指标与 7 个分析维度。",
        "将跨产线每周流量取数时长从约 1 人天缩短至 10 分钟。",
        "业务可基于统一口径快速发现异动并定位问题来源，发现机会点挖掘新增量。"
      ]
    },
    "meituan-board": {
      title: "频道数据看板建设",
      ...
      responsibilities: [
        {
          ...
          title: "背景与目标",
          items: [
            "背景：美团酒旅频道没有统一的数据看板，运营和产品定位问题要跨好几个系统拉数，决策效率很低",
            "目标：搭建一套能快速定位问题的监控体系，提升策略影响的评估效率，并对用户行为进行深度洞察"
          ]
        },
        {
          tone: "green",
          title: "频道总体流量转化",
          items: [
            "核心指标是 DAU 以及渠道来源；渠道来源分为金刚区、非金刚区、外投等",
            "监控频道页的整体曝光、访问用户规模，以及从曝光到意向、支付的核心转化漏斗，掌握宏观表现"
          ]
        },
        {
          tone: "orange",
          title: "频道重要埋点流量转化表现",
          items: [
            "对频道页内的关键功能模块（如一 tab、二 tab、搜索、橱窗区、猜喜、底 tab 等）进行独立监控",
            "分析各模块自身的曝光、点击及后续转化表现，洞察微观模块的效率"
          ]
        },
        {
          tone: "purple",
      ...
      results: [
        ...
        "为猜喜看板设置活动期 / 日常期双基准线，大促自动切换。猜喜双基准线上线后，日常策略迭代数据不再被异常流量干扰，监控体系长期稳定性提升。"
      ]
    },
    "meituan-mbr": {
    ...
    "meituan-plan": {
      ...
      responsibilities: [
        ...
        },
        {
          ...
          items: [
            ...
            "推动及时当地商品标签体系，联动算法将商品前置至猜喜顶端",
            "将「首页→猜喜→空搜→购买」优化为「首页→猜喜→购买」"
          ]
        },
        {
          tone: "purple",
      ...
      results: [
        "猜喜在及时当地场景转化率由 3.2% 提升至 12.6%，仅空搜用户被有效激活。",
    ...
    "meituan-rec": {
      ...
      results: [
        ...
        "按全量用户估算，预计能为美团到餐业务带来近 70 万的日活提升。",
  ...
  // 总分 = 学历×15% + 工作经验×30% + 技能×30% + 软技能×25%
  function computeOverallScore(scores) {
    try {
      const overall = Math.round(
        Number(scores.education.score) * 0.15
        + Number(scores.experience.score) * 0.30
        + Number(scores.skills.score) * 0.30
        + Number(scores.soft_skills.score) * 0.25
      );
  ...
  function renderMatchCard(jsonStr) {
    try {
      ...
      const computedOverall = computeOverallScore(scores);
      if (computedOverall !== null) {
        analysis.overall_score = computedOverall;
      }
